...
{
    auto year = boost::lexical_cast<int>(start_time_string.substr(0, 4));
    auto month = boost::lexical_cast<int>(start_time_string.substr(4, 2));
    auto day = boost::lexical_cast<int>(start_time_string.substr(6, 2));
    auto hour = boost::lexical_cast<int>(start_time_string.substr(8));
...
void GradsCtlParser::parseDataEndian(const std::string &endian_string) {
    if(endian_string == "big_endian") {
        grads_ctl_.data_endian_ = DataEndian::BigEndian;
    } else if(endian_string == "little_endian") {
